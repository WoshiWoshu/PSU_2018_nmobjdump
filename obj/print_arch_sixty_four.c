...
bool get_elf_fields_sixty_four(file_info_t *file_info,
     elf_fformat_t *elf_field)
{
    elf_field->elf_header = (Elf64_Ehdr *)file_info->mmaped_data;
    elf_field->fcontent = (Elf64_Shdr *)(file_info->mmaped_data +
    elf_field->elf_header->e_shoff);
    elf_field->name_sections = (char *)(file_info->mmaped_data +
    elf_field->fcontent[elf_field->elf_header->e_shstrndx].sh_offset);
    return (SUCCESS);
}

static void flag_f(const char *file, Elf64_Ehdr elf_header)
{
...
static void flag_s(Elf64_Shdr curr_section, Elf64_Ehdr *elf_header,
...
void arch_sixty_four(const char *file,
